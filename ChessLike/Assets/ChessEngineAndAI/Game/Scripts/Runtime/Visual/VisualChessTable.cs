...
namespace ChessEngine.Game
{
    ...
    public class VisualChessTable : MonoBehaviour
    {
        ...
        #endregion
        #region Position And Orientation Method(s)
        /// <summary>
        /// Returns the world space center position of the entire chess table
        /// by computing the average position of the 4 corner-most tiles' centers.
        /// </summary>
        /// <returns>The world space center position of the chess table.</returns>
        /// <exception cref="InvalidOperationException">Thrown if the VisualTiles array is not initialized or is empty.</exception>
        public Vector3 GetCenterPosition()
        {
            // Ensure the VisualTiles array is initialized and has valid dimensions.
            if (VisualTiles == null || VisualTiles.Length == 0 || VisualTiles[0].Length == 0)
            {
                // Throws an exception if VisualTiles is null or improperly configured.
                throw new InvalidOperationException("VisualTiles array is not initialized or is empty.");
            }

            // Get references to the corner-most tiles.
            VisualChessTableTile cornerA = VisualTiles[0][0];
            VisualChessTableTile cornerB = VisualTiles[0][VisualTiles[0].Length - 1];
            VisualChessTableTile cornerC = VisualTiles[VisualTiles.Length - 1][0];
            VisualChessTableTile cornerD = VisualTiles[VisualTiles.Length - 1][VisualTiles[0].Length - 1];

            // Calculate the world space positions of the four corners using GetCenterPosition from the tiles.
            Vector3 positionA = cornerA.GetCenterPosition();
            Vector3 positionB = cornerB.GetCenterPosition();
            Vector3 positionC = cornerC.GetCenterPosition();
            Vector3 positionD = cornerD.GetCenterPosition();

            // Compute the average position of the four corner points to get the table's center position.
            Vector3 centerPosition = (positionA + positionB + positionC + positionD) / 4f;
